...
### Status page visibility and the public home page

The app has a **public home page** (`/`) that lists status pages marked as "publish on home page". Visitors don't need to log in to see it.

**Status page privacy options:**
- **Published** — appears on the public home page (`/`) and is accessible via `/status/<slug>`
- **Unpublished** (private) — does not appear on the home page, but is still accessible at `/status/<slug>` if someone knows the URL

The "all monitors" status view (`/api/status` API endpoint) is **admin-only** — it requires authentication. The default dashboard at `/admin` is also admin-only. Managers and Users can log in to the `/admin` dashboard with read-only or limited edit access, but they cannot see the bulk all-monitors API endpoint.

**URLs summary:**
- `/` — public home page (lists published status pages)
- `/login` — login page (anyone can visit)
- `/admin` — admin dashboard (requires login)
- `/status/<slug>` — individual status pages (public, accessible to anyone with or without login)
...
- [ ] `ADMIN_PASSWORD` set to something you chose, not left to autogenerate (see "Set explicit admin credentials" above; check logs with `docker compose logs | grep -i password` if you're not sure)
- [ ] 2FA turned on for the admin account (Security tab in `/admin`)
- [ ] TLS terminated in front of the app (reverse proxy or LB), `TRUST_PROXY=1` and `COOKIE_SECURE=1` set
- [ ] Public home page (`/`) is reviewed — it lists all status pages marked as "publish on home page"; verify only the status pages you intend to share are published
- [ ] Individual status pages (`/status/<slug>`) are all intended to be public — if a monitor shouldn't be publicly visible at all, either unpublish its page or don't create it (this app doesn't support per-monitor public/private flags; see Limitations)
- [ ] `/api/status` (bulk all-monitors endpoint) is admin-only (verified in this version) and not exposed to the public
...
- **No per-monitor public/private visibility control.** Status pages can be published or unpublished (shown on the home page or hidden), but individual monitors within a status page can't be selectively hidden from authenticated users. If a monitor shouldn't be publicly visible at all, don't include it in any status page, or run a separate instance for internal monitors.
